...
class DefaultChannel(with_metaclass(TypeclassBase, ChannelDB)):
    ...
    def has_connection(self, subscriber):
        ...
        # ASH: May start mauling from here.
    ...
    def message_transform(self, msgobj, emit=False, prefix=True,
        ...
        #WHN: First %s is "channel name ", second is "name says, " message"
        if prefix:
    ...
    def channel_prefix(self, msg=None, emit=False):
        ...
        """
        #WHN: This controls how the channel's name appears with surrounding text.
        #WHN: I can probably change up some colors here based upon self.key's value.
        if self.key == "Newbie":
            return '' if emit else '|g[%s]|n: ' % self.key
        elif self.key == "Rage":
            return '' if emit else '|r[%s]|n: ' % self.key
        elif self.key == "Corinth":
            return '' if emit else '|w[%s]|n: ' % self.key
        elif self.key == "Gods":
            return '' if emit else '|c[%s]|n: ' % self.key
        else:
    ...
    def pose_transform(self, msgobj, sender_string):
        ...
        #WHN: First %s here is the channel name and the sender's name.
        else:
