    ...
    * Hook methods
    ...
    def at_look(self, target=None, session=None):
        ...
        if target and not is_iter(target):
            ...
            #                                 or "  %s" % (isess + 1), addr))
            result.append("\n\n |whelp|n - view more commands.")
            result.append("\n |wooc <Text>|n - ask for help on a public OOC channel (note, this will reveal your account name).")

            charmax = _MAX_NR_CHARACTERS if _MULTISESSION_MODE > 1 else 1

            if is_su or len(characters) < charmax:
                if not characters:
                    result.append("\n\n You don't have any characters yet. Use |w@charcreate <name>|n to create one.")
                else:
