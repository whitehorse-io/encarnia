...
class CmdCpAttr(ObjManipCommand):
    ...
    """
    key = "@cpattr"
    locks = "cmd:perm(cpattr) or perm(Immortals)"
    help_category = "Building"
...
class CmdMvAttr(ObjManipCommand):
    ...
    """
    key = "@mvattr"
    locks = "cmd:perm(mvattr) or perm(Immortals)"
    help_category = "Building"
...
class CmdCreate(ObjManipCommand):
    ...
    new_obj_lockstring = "control:id({id}) or perm(Wizards);delete:id({id}) or perm(Wizards)" # WHN: Trying to expand who can edit.
    ...
    def func(self):
        ...
        for objdef in self.lhs_objs:
            ...
            chan_string = "%s was created by %s." % (obj.name, caller.name)
            channel = ChannelDB.objects.get_channel("Building")
            channel.msg(chan_string)
        if string:
            caller.msg(string)
    ...
    def func(self):
        ...
        def delobj(objname, byref=False):
            ...
            if not okay:
                ...
                chan_string = "%s was deleted by %s." % (obj.name, caller.name)
                channel = ChannelDB.objects.get_channel("Building")
                channel.msg(chan_string)
                if had_exits:
...
class CmdDig(ObjManipCommand):
    ...
    Usage:
      @dig roomname[:typeclass] [= exit_to_there[;alias][:typeclass]][// exit_to_here[;alias][:typeclass]]

    Examples:
       @dig kitchen = north;n// south;s

    The example creates a room named kitchen to the north (with an alias of
    n for north) and a way back called south (alias s for south).
    """
    key = "@dig"
...
class CmdSetAttribute(ObjManipCommand):
    ...
    def set_attr(self, obj, attr, value):
        ...
        try:
            ...
            chan_string = "%s was changed by %s." % (obj.name, caller.name)
            channel = ChannelDB.objects.get_channel("Building")
            channel.msg(chan_string)
...
class CmdTypeclass(COMMAND_DEFAULT_CLASS):
    ...
    def func(self):
        ...
        chan_string = "%s had its typeclass changed by %s." % (obj.name, caller.name)
        channel = ChannelDB.objects.get_channel("Building")
        channel.msg(chan_string)
...
class CmdWipe(ObjManipCommand):
    ...
    """
    key = "@wipe"
    locks = "cmd:perm(wipe) or perm(Wizards)"
    help_category = "Building"
    ...
    def func(self):
        ...
        chan_string = "%s had its attributes wiped by %s." % (obj.name, caller.name)
        channel = ChannelDB.objects.get_channel("Building")
        channel.msg(chan_string)
...
class CmdSpawn(COMMAND_DEFAULT_CLASS):
    ...
    def func(self):
        ...
        for obj in spawn(prototype):
            ...
            chan_string = "%s was spawned by %s." % (obj.name, caller.name)
            channel = ChannelDB.objects.get_channel("Building")
            channel.msg(chan_string)
