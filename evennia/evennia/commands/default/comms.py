...
#WHN: Adding this to make these more flexible?
...
class CmdAddCom(COMMAND_DEFAULT_CLASS):
    ...
    # ASH: Mauling this from True to False.
    player_caller = False
...
class CmdCdestroy(COMMAND_DEFAULT_CLASS):
    ...
    key = "@cdestroy"
    help_category = "Comms"
    locks = "cmd:not pperm(channel_banned) and pperm(Immortals)"
...
#WHN: Gonna edit this to make it look better.
class CmdPage(COMMAND_DEFAULT_CLASS):
    """
    Send a private message to another character or characters (no need for them to
    be online).

    Usage:
      page[/switches] [player1, player2,... = message]
      msg[/switches] [player1, player2,... = message]

    Switch:
      last - shows who you last messaged
      list - show your last |wnumber|n of tells/pages (default)

    If no argument is given, you will get a list of your latest messages.
    """
    ...
    def func(self):
        ...
        if rstrings:
            self.msg("\n".join(rstrings))
        self.msg("|yYou paged %s |ythe following:|n \"%s\"|n" % (", ".join(received), message))
