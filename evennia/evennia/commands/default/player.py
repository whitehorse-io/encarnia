...
class CmdCharCreate(COMMAND_DEFAULT_CLASS):
    ...
    def func(self):
        ...
        # WHN: sanity checks, previously was ^[\A-z. \"\'\-']+$
        #if not re.findall(r"^[-\s\"\'a-zA-Z]+$", key) or not (0 < len(key) <= 25):
        if not re.findall(r"^[-\a-zA-Z][^\s]+$", key) or not (0 < len(key) <= 16):
            # this echoes the restrictions made by django's auth
            # module (except not allowing spaces, for convenience of
            # logging in).
            string = "\n\r Character names can be a maximum of 16 characters or less. Letters and hyphens (-) only."
            self.msg(string)
            return
        # strip excessive spaces in playername
        key = re.sub(r"\s+", " ", key).strip()
        #TCN: Capitalize it too.
        key = key.title()
        ...
        if desc:
            ...
            new_character.db.desc = "You see nothing special... yet!"
    ...
    def func(self):
        ...
        if show_session_data:
            ...
            for session in session_list:
                ...
                delta_conn = time.time() - session.conn_time
                player = session.get_player()
                puppet = session.get_puppet() # Gonna use this instead of the first one.
                if puppet:
                    table.add_row(utils.crop(puppet.key, width=25), #player.key.was player.name
                                  utils.time_format(delta_conn, 0),
                                  utils.time_format(delta_cmd, 1))
        is_one = nplayers == 1
