...
class CmdHelp(Command):
    ...
    def format_help_entry(title, help_text, aliases=None, suggested=None):
        ...
        string = _SEP + "\n"
        if title:
            string += "    |w%s|n" % title #WHN: Deleted "|C help for " intro, added spaces
        if aliases:
    ...
    def format_help_list(hdict_cmds, hdict_db):
        ...
        if hdict_cmds and any(hdict_cmds.values()):
            ...
            for category in sorted(hdict_cmds.keys()):
                string += "\n  |w%s|n:\n" % (str(category).title())
                string += "|G" + fill("|C, |G".join(sorted(hdict_cmds[category]))) + "|n\n" #ASH: Mauling code from here on.
            string += "\n|C------------------------------------------------------------------------------\n"
            string += "   Also see: |whelp index|C and |whelp lore|C.\n"
            string += "------------------------------------------------------------------------------|n"
        # if hdict_db and any(hdict_db.values()): #ASH: Mauling this out.
