...
def list_to_string(inlist, endsep="and", addquote=False):
    ...
    if addquote:
        if len(inlist) == 1:
            #return "\"%s\"" % inlist[0]
            here_list = "\"%s\"" % inlist[0]
            if not here_list[2].isupper():
                here_list = here_list.capitalize()
            return here_list

        #return ", ".join("\"%s\"" % v for v in inlist[:-1]) + "%s %s" % (endsep, "\"%s\"" % inlist[-1])
        here_list = ", ".join("\"%s\"" % v for v in inlist[:-1]) + "%s %s" % (endsep, "\"%s\"" % inlist[-1])
        if not here_list[2].isupper():
            here_list = here_list.capitalize()
        return here_list
    else:
        if len(inlist) == 1:
            #return str(inlist[0])
            here_list = str(inlist[0])
            if not here_list[2].isupper():
                here_list = here_list.capitalize()
            return here_list
        #return ", ".join(str(v) for v in inlist[:-1]) + "%s %s" % (endsep, inlist[-1])
        here_list = ", ".join(str(v) for v in inlist[:-1]) + "%s %s" % (endsep, inlist[-1])
        if not here_list[2].isupper():
            here_list = here_list.capitalize()
        return here_list
...
def at_search_result(matches, caller, query="", quiet=False, **kwargs): #WHN This is a complicated one...
    """
    ...
    if not matches:
        # no results.
        error = kwargs.get("nofound_string") or ("You could not find '%s'." % query) # WHN: a _ was before the new string?
        matches = None
    elif len(matches) > 1:
        error = kwargs.get("multimatch_string") or ("You note more than one %s and choose the first one." % query)
        # error = kwargs.get("multimatch_string") or \
        #         _("There is more than one %s here, to manipulate a different %s use 2-%s etc." % (query, query, query))
        ...
        matches = matches[0] # WHN: I think this'll use the first object?
    else:
        # exactly one match
        matches = matches[0]
