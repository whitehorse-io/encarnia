...
from evennia.utils.utils import (variable_from_module, lazy_property,
                                 make_iter, to_unicode, calledby, is_iter, list_to_string)
...
class DefaultObject(with_metaclass(TypeclassBase, ObjectDB)):
    ...
    def msg(self, text=None, from_obj=None, session=None, options=None, **kwargs):
        ...
        """
        # try send hooks
        #ASH: Going to experiment with just commenting this out for now.
        # if from_obj:
        #     try:
    ...
    def announce_move_from(self, destination, msg=None, mapping=None):
        ...
        """
        #TCN: changing a lot of this in exits.py instead of displaying it here.
        if not self.location:
            return
        if msg:
            string = msg
        else:
            #string = "{object} leaves {origin}, heading for {destination}."
            currentroom = str(self.location)
            string = "{object} departs from %s, heading {exit}." % (str.lower(currentroom))
    ...
    def announce_move_to(self, source_location, msg=None, mapping=None):
        ...
        # TCN: Altered these to use proper capitalization etc.
        if source_location:
            if msg:
                string = msg
            else:
            ...
            string = "{object} arrives at %s." % str.lower(currentroom)
        ...
        mapping.update({
                #"object": self,
                "object": str.capitalize(str(self.name)),
    ...
    def return_appearance(self, looker):
        ...
        # TCN: Changing this stuff around.
        string = "|y%s.|n\n" % self.get_display_name(looker)
        desc = self.db.desc
        #string = "|c%s|n\n" % self.get_display_name(looker)
        #desc = self.db.desc

        # WHN: Trying to change the display of room exits here.  Not sure I want a separate line for "also here".

        if desc:
            string += "%s" % desc
        # WHN: I should use a counting function, if there's two or less it uses the current format, if there's three or more it ends with "and".

        #if len(users) + len(things) < 2 and len(users) + len(things) > 0:
        #    string += "\n" + ", ".join(users + things) + " is here."
        #elif users or things:
        #    string += "\n" + ", ".join(users + things) + " are here."
        if len(users + things) > 1:
            #string += "\n|wYou see:|n " + ", ".join(users + things)
            string += "\n" + list_to_string(users + things) + " are here."
        # elif len(users + things) == 2:
        #     if not string[2].isupper():
        #         string = string.capitalize()
        elif users or things:
            string += "\n" + list_to_string(users + things) + " is here."

        if exits and len(exits) > 1:
            string += "\n|wYou see exits leading:|n " + ", ".join(exits) + "."
        elif exits:
            string += "\n|wYou see a single exit:|n " + ", ".join(exits) + "."  # WHN: Will this come out right?
        return string
